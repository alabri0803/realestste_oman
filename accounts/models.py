...
class CustomUser(AbstractUser):
    ...
    def clean(self):
        ...
        if self.user_type in [self.UserTypes.INVESTOR, self.UserTypes.TENANT_COMPANY] and not self.commercial_register:
            raise ValidationError({'commercial_register': _('مطلوب إدخال السجل التجاري لهذا النوع من الحسابات')})
...
class UserProfile(models.Model):
    ...
    def __str__(self):
        return f"ملف تعريفي لـ {self.user.arabic_name}"
...
class ActivityLog(models.Model):
    ...
    def __str__(self):
        if self.user:
            return f"{self.user.arabic_name} - {self.action}"
        return f"نشاط مجهول - {self.action}"